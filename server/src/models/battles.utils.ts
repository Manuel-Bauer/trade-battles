...
export function calculateStockStats(transactionsPerStock, currentPrices) {
  ...
  Object.entries(transactionsPerStock).map((transaction) => {
    const [symbol, value] = transaction;
    ...
    const ytdChange = quote.ytdChange;

    const totalValue = currentPrice * owning;
    const averageBuyIn = calculateAverage(
      value.filter((transaction) => transaction.action == 'BUY').map((transaction) => transaction.price)
    );
